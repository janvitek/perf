...
public abstract class Job {
  ...
  public static Job build(App.Conf conf) {
    if (conf.kind.equals("plain")) return new Plain(conf);
    else if (conf.kind.equals("local")) return new Local(conf);
    else if (conf.kind.equals("cloud")) return new Cloud(conf);
  ...
  static class Plain extends Job {
    ...
    public void run() throws Exception {
      seconds.add(timeExec(new ProcessBuilder(
          concat(new String[] { "java", "-cp", conf.jar, conf.main, "-input=" + conf.input, "-output=" + conf.output },
              conf.arguments))));
    }
  }
